...
# MoDSE
    ...
    with open(PHV_file_path, 'a') as phv_file, open(ADRS_file_path, 'a') as adrs_file:
        ...
        for it in (bar := tqdm(range(iteration))):
            # current_Pareto_front = randomSampleParetoOptimal(sample_num, current_Pareto_front)
            for _ in range(sample_num):
                ...
                input_tensor = torch.tensor(parameter_vector).unsqueeze(0).to("cpu")

                pred_ipc, pred_power, pred_area, _, _, _ = predict(dataset_name, input_tensor, model_name)
            ...
            # print(f"Baseline model finished Pareto optimal set search...")
            # print(f"Calculating ture performance metrices...")
            ...
            # print(final_Pareto_optimal_set)

            # Estimate the Pareto optimal set DSE framework searched 
